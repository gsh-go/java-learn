...
/**
 * @Author: gsh
 * @Date: Created in 2018/9/21 8:38
 * @Description:
 */
public class ClassPathXMLApplicationContext {
    ...
    List<IocBean> beanList = new ArrayList<IocBean>();
    ...
    public void propertyAnnotation(Object bean){
        try {
            ...
            for(PropertyDescriptor proderdesc : ps){
                ...
                //判断set方法是否定义了注解
                if(setter!=null && setter.isAnnotationPresent(IocResource.class)){
                    //获取当前注解，并判断name属性是否为空
                    IocResource resource = setter.getAnnotation(IocResource.class);
                    String name ="";
                    Object value = null;
    ...
    public void fieldAnnotation(Object bean){
        try {
            ...
            for(Field f : fields){
                if(f!=null && f.isAnnotationPresent(IocResource.class)){
                    IocResource resource = f.getAnnotation(IocResource.class);
                    String name ="";
                    Object value = null;
                    if(resource.name()!=null&&!"".equals(resource.name())){
                        name = resource.name();
